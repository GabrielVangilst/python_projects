...
class Ball:
    ...
    def animation(self, screen_height, player1, player2):
        ...
        # if the ball hits the top or bottom of the screen it will bounce in the opposite direction
        if self.image.top <= 0 or self.image.bottom >= screen_height:
            self.velocity_y *= -1
        ...
        if self.image.colliderect(player1):
            ...
            self.velocity_x = abs(math.cos(math.radians(bounce_angle)) * velocity) * -1
            self.velocity_y = math.sin(math.radians(bounce_angle)) * velocity * -1
        ...
        if self.image.colliderect(player2):
            ...
            scaled_diff = diff / 70
            bounce_angle = max_angle * scaled_diff
            new_velocity_x = abs(math.cos(math.radians(bounce_angle)) * velocity)
            self.velocity_x = abs(math.cos(math.radians(bounce_angle)) * velocity)
            self.velocity_y = math.sin(math.radians(bounce_angle)) * velocity * -1
